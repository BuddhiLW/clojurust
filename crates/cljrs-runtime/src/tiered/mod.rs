...
use crate::tiered::ir_interp::eager_lower_fn;
use std::sync::Arc;
...
pub fn mark_compiler_ready(globals: &Arc<GlobalEnv>) -> bool {
    ...
    jit_state::set_bootstrap_arity_watermark(crate::interp::arity::next_arity_id());
    true
}

pub fn standard_env_minimal() -> Arc<GlobalEnv> {
    crate::interp::standard_env_minimal(
        Some(eval),
        Some(apply::call_cljrs_fn),
        Some(eager_lower_fn),
    )
}
...
pub fn standard_env_minimal_no_ir() -> Arc<GlobalEnv> {
    crate::interp::standard_env_minimal(Some(eval), Some(apply::call_cljrs_fn), None)
}
