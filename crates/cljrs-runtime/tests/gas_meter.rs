...
fn ordinary_eval_is_unmetered() {
    assert_eq!(
        cljrs_runtime::interp::eval::eval(&form("(+ 1 2)"), &mut env()).unwrap(),
        Value::Long(3)
    );
}
...
fn zero_credits_exhaust_before_evaluation() {
    assert!(matches!(
        cljrs_runtime::interp::eval::eval_with_gas(&form("1"), &mut env(), 0),
...
fn recursive_work_exhausts_and_cannot_be_caught() {
    ...
    assert!(matches!(
        cljrs_runtime::interp::eval::eval_with_gas(&form(source), &mut env(), 40),
        Err(EvalError::GasExhausted)
    ));
}
