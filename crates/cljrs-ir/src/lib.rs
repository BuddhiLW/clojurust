...
pub enum Inst {
    ...
    ///
    /// A name starting with `.` is a method-interop marker, not a function
    /// name: `(.method target args…)` lowers to
    /// `CallDirect(dst, ".method", [target, args…])`.  The IR interpreter
    /// routes these to the tree-walker's method dispatch; native codegen
    /// rejects the unknown name, so interop-bearing functions decline JIT
    ...
    /// Bind a VarId to the region handle inherited from the caller.
    ///
    /// Emitted at the entry block of a region-parameterised callee variant
    /// (produced by stage 4 of the escape-optimisation pipeline).  The bound
    /// VarId is referenced as the `region` operand of subsequent
    /// `RegionAlloc` instructions in the callee body.
    ///
    /// In compiled (AOT/JIT) code the region arrives as a hidden trailing
    /// function parameter — the caller's `CallWithRegion` passes its region
    /// handle, and the callee bump-allocates into it directly.  The IR
    /// interpreter instead binds this to nil and inherits the region via the
    /// thread-local region stack.
    RegionParam(VarId),

    /// Direct call to a region-parameterised callee variant.
    ///
    /// `(dst, target_name, args, region)` — semantically identical to
    /// `CallDirect`, but the call site has wrapped itself in
    /// `RegionStart`/`RegionEnd` and passes its region handle (the
    /// `RegionStart` dst) so the callee's `RegionAlloc` instructions can
    /// allocate into the caller's region.  In compiled code the region is
    /// threaded as a hidden trailing argument (bound to the callee's
    /// `RegionParam`); the IR interpreter relies on the thread-local region
    /// stack instead.  Generated by the stage-4 cross-function
    /// region-promotion pass.
    CallWithRegion(VarId, Arc<str>, Vec<VarId>, VarId),

    // ── Async instructions ───────────────────────────────────────────────
    /// Yield point: await a `Future`/`Promise` inside an `^:async` function.
    ///
    /// `(dst, src)` — `src` holds a `Value::Future`/`Value::Promise`; `dst`
    /// receives the resolved value.  In the IR interpreter this falls back to
    /// a blocking deref; in `eval_async` it cooperatively yields.
    Await { src: VarId, dst: VarId },

    /// Spawn an `^:async` function body as a `LocalSet` task.
    ///
    ...
    ///
    /// `(dst, chan)` — parks the async task until a value is available; `dst`
    /// receives the value (or `nil` on a closed channel).
    ChanTake { chan: VarId, dst: VarId },

    /// Put a value into a channel (async).
    ///
...
impl IrFunction {
    ...
    /// Whether this function is a region-parameterised variant — its entry
    /// block binds a [`Inst::RegionParam`], so compiled code receives the
    /// caller's region as a hidden trailing parameter.
        ...
        self.blocks
            ...
            .map(|entry| {
                entry
                    .insts
                    .iter()
                    .any(|i| matches!(i, Inst::RegionParam(_)))
            })
            .unwrap_or(false)
    }
...
impl Inst {
    ...
    pub fn effect(&self) -> Effect {
        match self {
            ...
            Inst::RegionParam(..) => Effect::Pure,
            Inst::CallWithRegion(..) => Effect::UnknownCall,
        }
    }
    ...
    pub fn dst(&self) -> Option<VarId> {
        match self {
            ...
            | Inst::Phi(v, _)
            | Inst::RegionStart(v)
            | Inst::RegionAlloc(v, _, _, _)
            | Inst::RegionParam(v)
            | Inst::CallWithRegion(v, _, _, _) => Some(*v),
    ...
    pub fn uses(&self) -> Vec<VarId> {
        match self {
            ...
            Inst::RegionParam(..) => vec![],
            Inst::CallWithRegion(_, _, args, region) => {
                let mut v = args.clone();
                v.push(*region);
                v
            }
            Inst::Await { src, .. } => vec![*src],
            Inst::Spawn { fn_reg, args, .. } => {
                let mut v = vec![*fn_reg];
                v.extend(args);
                v
            }
