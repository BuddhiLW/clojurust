...
[`docs/archive/jit-plan.md`](docs/archive/jit-plan.md). Milestones below map 1:1 to that
document's layers.
...
- [ ] Known pre-existing bug surfaced under `CLJRS_EAGER_LOWER=1`: Tier-1 IR closures in clojure.test patterns fail with `Arity { name: "<ir-closure>", expected: 2, got: 0 }` (`cargo test -p cljrs-stdlib --test compiler_clojure_tests` with `CLJRS_EAGER_LOWER=1`). The seq-driven JIT loop bug is covered by `execution_tier_parity.rs` and its subprocess timeout.
...
Handoff + design doc: [`docs/archive/wasm-aot-plan.md`](docs/archive/wasm-aot-plan.md).
...
- [~] Language Server Protocol (LSP) implementation for editor support
  ...
  - [ ] v2 (semantic, evaluator-backed via `cljrs_runtime::env::GlobalEnv`): hover, completion,
...
## cljrs-net — QUIC + HTTP/3 (`docs/archive/quic-http3-integration-plan.md`)
