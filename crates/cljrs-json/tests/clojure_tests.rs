...
const TEST_NSES: &[&str] = &["clojure.data.json-test"];
...
fn run_clojure_json_tests() {
    ...
    cljrs_json::register(&mut registry);
    ...
    for ns in TEST_NSES {
        eprintln!("[json-tests] running {ns}");
    ...
    eprintln!(
        "[json-tests] totals: {total_test} tests, {total_pass} pass, \
