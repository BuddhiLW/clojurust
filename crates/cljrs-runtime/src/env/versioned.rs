...
pub fn ensure_versioned_ns_loaded(
    ...
    let should_load = crate::env::loader::claim_or_wait(globals, &versioned_ns_name)?;
    if !should_load {
        return Ok(versioned_ns_name);
    }
...
fn do_versioned_load(
        ...
        .and_then(|v| crate::env::dynamics::deref_var(&v));
    {
        ...
        for form in forms {
            ...
            globals
                .eval(&form, &mut env)
                .map_err(|e| crate::env::loader::annotate(e, versioned_ns_name))?;
        }
    }
...
fn fetch_versioned_source(
    ...
    let src_paths = globals.source_paths.read().unwrap().clone();
    let (_, file_path) =
        crate::env::loader::find_source_file(&rel_path, &src_paths).ok_or_else(|| {
            EvalError::Runtime(format!(
