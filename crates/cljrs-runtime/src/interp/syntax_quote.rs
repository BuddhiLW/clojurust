...
use crate::builtins::form::{expand_reader_conds_cow, form_to_value, select_reader_cond};
...
fn sq_form(
    ...
    match &form.kind {
        ...
        FormKind::Unquote(inner) => crate::interp::eval::eval(inner, env),
    ...
    for f in forms.iter() {
        match &f.kind {
            FormKind::UnquoteSplice(inner) => {
                ...
                let v = crate::interp::eval::eval(inner, env)?;
                let items = crate::interp::destructure::value_to_seq_vec(&v);
...
fn qualify_symbol(
    ...
    if let Some(base) = s.strip_suffix('#') {
        let generated = gensyms.entry(s.to_string()).or_insert_with(|| {
            let n = crate::env::policy::next_transaction_gensym()
